...
starwars %>%
  dplyr::filter(species == "Human", homeworld == "Naboo")
```
...
starwars %>%
  dplyr::filter(species == "Human", homeworld == "Naboo", hair_color == "brown")
...
starwars %>%
  dplyr::filter(species == "Human", homeworld == "Naboo", hair_color == "brown", !is.na(mass))
...
starwars %>%
  dplyr::select(homeworld, everything())
```
...
starwars %>%
  dplyr::mutate(combined = height + mass)
```
...
starwars %>%
  dplyr::mutate(combined = height + mass, divided = height/mass)
```
...
data.example %>%
  dplyr::summarise(mean.wmc = mean(wmc.score), mean.ltm = mean(ltm.score))

# I want to get the mean wmc.score called "mean.wmc" & the standard deviation of wmc.score called "sd.wmc"
data.example %>%
  dplyr::summarise(mean.wmc = mean(wmc.score), sd.wmc = sd(wmc.score))
```
...
starwars %>%
  ...
  dplyr::summarise(mean.height = mean(height))
```
...
starwars %>%
  dplyr::group_by(species) %>%
  dplyr::summarise(mean.height = mean(height, na.rm = TRUE))
```
...
starwars %>%
  ...
  dplyr::summarise(eyecolor.height = mean(height)) %>%
  dplyr::slice_max(eyecolor.height, n = 2)
```
...
Seatbelts<-Seatbelts%>%
  tidyr::pivot_longer(front:rear, names_to = "Location", values_to = "Deaths")
...
ChickWeight<-ChickWeight %>%
  tidyr::pivot_wider(names_from = "Time", values_from = "weight")
